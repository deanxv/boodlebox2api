...
var (
	BBCookies    []string   // 存储所有的 cookies
	cookiesMutex sync.Mutex // 保护 BBCookies 的互斥锁
)
...
func InitSGCookies() {
	...
	// 从环境变量中读取 BB_COOKIE 并拆分为切片
	cookieStr := os.Getenv("BB_COOKIE")
	if cookieStr != "" {

		for _, cookie := range strings.Split(cookieStr, ",") {
			BBCookies = append(BBCookies, cookie)
		}
	}
}
...
// GetSGCookies 获取 BBCookies 的副本
func GetBBCookies() []string {
	//cookiesMutex.Lock()
	//defer cookiesMutex.Unlock()

	// 返回 BBCookies 的副本，避免外部直接修改
	cookiesCopy := make([]string, len(BBCookies))
	copy(cookiesCopy, BBCookies)
	return cookiesCopy
}
...
func NewCookieManager() *CookieManager {
	...
	// 遍历 BBCookies
	for _, cookie := range GetBBCookies() {
